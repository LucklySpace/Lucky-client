...
export const useChatStore = defineStore(
  ...
  () => {
    ...
    const getChatById = computed(
      () => (id: string | number) => chatList.value.find((c: Chats) => c.chatId === id) ?? null
    );
    ...
    const getTotalUnread = computed(() =>
      chatList.value.reduce((s, c) => (c.isMute === 0 ? s + (c.unread || 0) : s), 0)
    );
     ...
     * @param chatType 会话类型
     */
    const handleChangeCurrentChatByTarget = async (
      targetInfo: { [key: string]: string | number },
      chatType: number
    ) => {
    ...
    const handleDeleteChat = async (chat: Chats) => {
      ...
      if (currentChat.value?.chatId === chat.chatId) {
        // 清空当前会话及其相关的消息/成员/草稿等 store 数据
        currentChat.value = null;
        try {
          // 清空当前消息区
          messageList.value = [];
        ...
        try {
          // 清空当前群成员缓存
          // 兼容数组或对象形态
          if (Array.isArray(currentChatGroupMemberMap.value)) {
    ...
    const uploadAndSendFile = async (file: File, currentChat: any, contentType: number) => {
      ...
      const form = handleCreateMessageContext({ ...uploadRes, size: file.size, suffix: ext }, currentChat, contentType);
