<template>
  <div aria-label="视频列表" class="video-grid" role="list">
    ...
    <div v-for="userId in participantIds" :key="userId" :aria-label="`视频单元：${userDisplayName(userId)}`"
      class="video-cell" role="listitem">
      <!-- video 元素：大小由 .video-window 控制（100% 宽度 + aspect） -->
      <video :ref="el => onVideoRef(el, userId)" :aria-label="`视频：${userDisplayName(userId)}`" :muted="userId === id"
        autoplay class="video-window" controlslist="nodownload noremoteplayback" playsinline></video>
