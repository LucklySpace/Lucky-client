<template>
  <div class="meeting-root" role="application">
    <transition name="fade-up">
      <div :style="{ backgroundColor: showControls ? '#eee' : 'transparent' }" class="video-call-header"
        data-tauri-drag-region role="region">
    ...
    <el-container class="meeting-page" @mouseenter="onUserEnter" @mouseleave="onUserLeave" @mousemove="onUserActivity">
      ...
      <div v-if="isMobile" :class="['chat-backdrop', { 'is-open': !isChatCollapsed }]" aria-hidden="true"
        @click="toggleChat && toggleChat()"></div>

      <!-- 聊天侧栏（响应式：大屏是常规 aside；小屏是抽屉） -->
      <el-aside :class="['chat-section', { 'chat-drawer': isMobile, open: !isChatCollapsed }]"
        :style="{ display: isChatCollapsed && !isMobile ? 'none' : '' }">
        <ChatPanel v-model:newMessage="newMessage" :messages="messages" :participants="participantsMap"
          :user-id="userStore.userId" @send-message="sendMessage" />
      </el-aside>

      <!-- 底部控制栏 -->
      <transition name="fade-up">
        <ControlsBar v-show="showControls" :is-chat-collapsed="isChatCollapsed" :is-muted="isMuted"
          :is-video-off="isVideoOff" @toggle-mute="toggleMute" @toggle-video="toggleVideo" @hang-up="handUp"
          @toggle-chat="toggleChat" @mouseenter.native="onControlsEnter" @mouseleave.native="onControlsLeave" />
      </transition>
    </el-container>
  </div>
</template>
