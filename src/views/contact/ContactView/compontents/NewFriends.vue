          ...
          <li v-for="req in requests" :key="req.id" class="friend-requests__item">
            ...
            <div class="friend-requests__actions">
              ...
              <div v-else class="friend-requests__status" :class="getStatusClass(req)">
                <el-icon v-if="isAccepted(req)" :style="{ color: 'rgb(133, 206, 97)' }">
                  <CircleCheck />
                </el-icon>
                <el-icon v-else :style="{ color: 'rgb(166, 169, 173)' }">
                  <CircleClose />
                </el-icon>
                <span v-if="isAccepted(req)" :style="{ color: 'rgb(133, 206, 97)' }">
                  {{ $t("business.friend.status.accepted") }}
                </span>
                <span v-else :style="{ color: 'rgb(166, 169, 173)' }">
                  {{ $t("business.friend.status.rejected") }}
                </span>
              </div>
            </div>
          </li>
...
<script lang="ts" setup>
  ...
  import { Check, CircleCheck, CircleClose, Close, Refresh } from "@element-plus/icons-vue";
