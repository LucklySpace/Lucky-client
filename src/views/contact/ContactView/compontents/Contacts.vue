<template>
  ...
  <div :aria-label="$t('contacts.contactInfo')" class="contact-page-shell" role="region">
    <!-- 卡片：适度放大，阴影与圆角 -->
    <el-card v-show="hasFriend" :body-style="{ padding: '18px' }" aria-live="polite" class="contact-card"
      shadow="hover">
      <!-- 顶部：头像 + 基本信息 -->
        ...
        <el-col :span="18" class="meta-col">
          <div class="meta-row">
            <div class="name-and-gender">
              ...
              <!-- 性别 svg（使用 sprite） -->
              <svg v-if="friendInfo.gender === 1" aria-hidden="true" class="gender-icon no-select">
                <use xlink:href="#icon-nanxing" />
              </svg>
              <svg v-else-if="friendInfo.gender === 0" aria-hidden="true" class="gender-icon no-select">
            ...
            <div class="sub-info">
              <span v-if="friendInfo.friendId" class="muted">{{ $t("contacts.idLabel") }} {{ friendInfo.friendId
                }}</span>
            </div>
          </div>
        </el-col>
      ...
      <div class="info-section">
        <div v-if="friendInfo.name" class="info-row">
          <strong class="no-select">{{ $t("profile.remark") }}</strong>
...
async function saveRemark() {
  ...
  if (!id) return;
  if (!next) {
    ElMessage.warning($t("errors.remark.empty") as any);
    return;
  }
  if (next.length > MAX_REMARK_LEN) {
    ElMessage.error($t("errors.remark.tooLong", { max: MAX_REMARK_LEN }) as any);
    return;
  }
  try {
