<template>
  <div ref="containerRef" :class="{ 'hide-scrollbar': hideScrollbar }" class="virtual-list-container"
       @mouseenter="handleMouseEnter" @mouseleave="handleMouseLeave" @scroll.passive="handleScroll">
    <n-flex v-if="!isLoadingMore && isLast" class="box-border absolute-x-center pt-10px" justify="center">
      <span class="text-(12px #909090)">以下是全部消息内容</span>
    </n-flex>
    <n-flex v-if="isLoadingMore && !isLast" class="box-border absolute-x-center pt-10px" justify="center">
      <img alt="" class="size-16px" src="@/assets/img/loading.svg" />
    ...
    <div :style="{ transform: `translateY(${offset}px)` }" class="virtual-list-content">
      <div v-for="item in visibleData" :id="`item-${item.message?.id}`" :key="item.message?.id"
           :data-item-index="item._index">
        <slot :index="item._index" :item="item"></slot>
      </div>
    </div>
  </div>
</template>
