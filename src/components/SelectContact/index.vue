    ...
    <el-row :gutter="20">
      <el-col :span="12">
        <div class="contact-list">
          ...
          <!-- 虚拟列表容器 -->
          <div ref="boxRef" :aria-label="$t('components.selectContact.list')" class="vl-box" tabindex="0">
            <!-- 总高度占位 -->
            <div :style="{ height: totalHeight + 'px', position: 'relative' }">
              <div v-for="(row, idx) in visibleRows" :key="row.friendId" :style="{
                height: rowHeight + 'px',
                position: 'absolute',
                left: 0,
                right: 0,
                ...
                <div class="selection-item">
                  <el-row style="height: 40px; align-items: center">
                    <el-col :span="3" style="display: flex; align-items: center; justify-content: center">
                      ...
                      <el-checkbox :aria-label="$t('components.selectContact.title')"
                        :model-value="!!selectedContacts[row.friendId]" @change="val => onCheckboxToggle(row, val)" />
                    </el-col>
      ...
      <el-col :span="12">
        <div class="selected-list">
          {{ $t("components.selectContact.selected", { count: selectItem.length }) }}
            ...
            <el-row :gutter="20">
              <el-col v-for="(item, index) in selectItem" :key="item.friendId" :span="8">
                <div style="position: relative; display:flex; justify-content: center">
                  ...
                  <el-button :aria-label="`移除 ${item.name}`" circle class="cancel-el-btn" icon=""
                    @click.stop="cancelSelection(item)" @keydown.space.prevent="cancelSelection(item)"
                    @keydown.enter.prevent="cancelSelection(item)">
...
function onCheckboxToggle(item: any, checked?: any) {
  ...
  // 如果 change 事件传入明确的布尔值则以该值为准；否则切换
